...
def series_to_supervised(df, out_col, n_in=56, n_out=56, lag=56, dropnan=True,
...
def prepare_data(data_opt, reframed, original_indices):
    ...
    n_train_hours = int(data_opt['tr_per'] * len(original_indices)) # Adjusted based on original indices post NaN removal
...
csv_file = "data\Dataset with WPD.csv"
...
columns_to_keep = ['MinuteOfDay', 'Day', 'Month', 'Tair', 'WS', 'RH', 'GHI_1', 'GHI_2', 'GHI_3', 'GHI_4', 'SS1', 'SS2',
                   'SS3', 'SS4', 'P']
df = df[columns_to_keep]
data_opt = {
    'tr_per': 0.75,  # Training percentage
    'val_per': 0.1,  # Validation percentage
    'n_in': 112,  # Original number of input timesteps
...
supervised_df, original_indices = series_to_supervised(df, 'P', data_opt['n_in'], data_opt['n_out'], data_opt['lag'], True, 'DateTime', False)
train_X, train_y, val_X, val_y, test_X, test_y, scaler_X, scaler_y, train_indices, val_indices, test_indices = prepare_data(data_opt, supervised_df, original_indices)

# section 5: Define and train the LSTM model-----------------------------------
def lstm_model(n_in, n_features, learning_rate=0.001):
    model = Sequential()
    model.add(LSTM(50, activation='relu', return_sequences=False, input_shape=(n_in, n_features)))
    model.add(Dropout(0.0))  # Dropout to prevent overfitting 
    # model.add(LSTM(112))  # Additional LSTM layer if needed
    model.add(Dense(50, activation='relu'))  # New dense layer with 14 units and ReLU activation
    ...
    # model.add(LSTM(112))  # Additional LSTM layer if needed
    model.add(Dense(20, activation='relu'))  # New dense layer with 14 units and ReLU activation
    model.add(Dense(data_opt['n_out']))  # Output layer
...
history = model.fit(train_X, train_y, epochs=2, batch_size=40, validation_data=(val_X, val_y))
...
# Print the minimum of inv_yhat
min_inv_yhat = np.min(inv_yhat)
print('Minimum of inv_yhat: %.3f' % min_inv_yhat)
...
start_date = pd.to_datetime('2023-10-07')  # Example start date, modify as needed
